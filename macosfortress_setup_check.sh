...
LAUNCHD_PLISTS=( \
	...
	org.squid-cache.squid-rotate.plist \
        homebrew.mxcl.squid.plist \
        homebrew.mxcl.privoxy.plist \
        homebrew.mxcl.clamd.plist \
        homebrew.mxcl.freshclam.plist \
        homebrew.mxcl.ClamavScanSchedule.plist \
        homebrew.mxcl.ClamavScanOnAccess.plist \
    )
...
sudo pfctl -si
less /var/log/pf.log
sudo /usr/local/bin/gpg --homedir /var/root/.gnupg --list-keys | grep -A2 -B1 -i dshield.org
...
sudo /usr/local/bin/gpg --homedir /var/root/.gnupg --list-keys | grep -A2 -B1 -i hpHosts
...
# squid
if [[ `$SUDO $LSOF -i ':3128' | $TAIL -1` && `$PS -ef | $GREP "/usr/local/sbin/squid -s" | $EGREP -v '(grep|daemondo)' | $WC -l` -eq 1 ]]; then
...
sudo squid -k check
sudo less /usr/local/var/squid/logs/cache.log
...
sudo less /usr/local/var/log/privoxy/logfile
...
sudo ps -f `cat /usr/local/var/run/nginx/nginx-adblock2privoxy.pid`
